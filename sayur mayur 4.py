...
class Kebun:
    ...
    def create_sayuran(self, nama, teknik, jumlah, hari, harga, posisi='akhir', posisi_nomor=None, data=True):
    ...
    def search_sayuran(self, attribute, value):
        current = self.head
        items_ada = []
        while current is not None:
            if (attribute == 'nomor' and current.nomor_sayuran == value) or \
            (attribute == 'nama' and current.data['nama'] == value):
                items_ada.append(current)
            current = current.next
        if items_ada:
            print(f"Hasil pencarian untuk {attribute} = {value}:\n")
            for item in items_ada:
                print(f"Nomor: {item.nomor_sayuran}")
                print(f"Nama: {item.data['nama']}")
                print(f"Teknik: {item.data['teknik']}")
                print(f"Jumlah: {item.data['jumlah']}")
                print(f"Hari: {item.data['hari']}")
                print(f"Harga: {item.data['harga']}\n")
        else:
            print(f"Sayuran dengan {attribute} = {value} tidak ditemukan\n")
            
    def data_sayuran(self):
        sayuran = [
            ("tomat", "tanam tanah", 100, "30 hari", 2000),
            ("selada", "hidroponik", 50, "45 hari", 15000),
            ("bayam", "hidroponik", 200, "25 hari", 10000),
        ]
        for nama, teknik, jumlah, hari, harga in sayuran:
            self.create_sayuran(nama, teknik, jumlah, hari, harga, data=False)
    ...
    def menu(self):
        while True:
            ...
            if pilihan == '1':
                ...
                print("Pencarian Sayuran")
                attribute = input("Cari berdasarkan (nomor/nama): ").lower()
                if attribute == "nomor":
                    value = int(input("Masukkan nomor sayuran: "))
                    self.search_sayuran(attribute, value)
                elif attribute == "nama":
                    value = input("Masukkan nama sayuran: ")
                    self.search_sayuran(attribute, value)
                else:
                    print("Pencarian hanya berdasarkan nomor atau nama\n")
